...
// Improved direct control approach without overriding core functions
document.addEventListener('DOMContentLoaded', function() {
    ...
    setTimeout(function() {
        ...
        // Get game canvas or container for focus
        const gameArea = document.getElementById('gameCanvas') || 
                          document.getElementById('gameContainer') || 
                          document.querySelector('.game-container');
        
        // Enhanced arrow controls
        function setupArrowControl(button, direction, keyCode) {
            // Create new button to clear listeners
            const newButton = button.cloneNode(true);
            button.parentNode.replaceChild(newButton, button);
            
            // Touch start - begin repeating movement
            newButton.addEventListener('touchstart', function(e) {
                ...
                // Set up interval for repeated movement
                movementInterval = setInterval(function() {
                    // Focus game area if possible
                    ...
                    // Try direct movement function calls as fallback
                    if (direction === 'ArrowLeft' && typeof movePieceLeft === 'function') {
                        movePieceLeft();
                    } else if (direction === 'ArrowRight' && typeof movePieceRight === 'function') {
                        movePieceRight();
                    } else if (direction === 'ArrowUp' && typeof rotatePiece === 'function') {
                        rotatePiece(1);
                    } else if (direction === 'ArrowDown' && typeof movePieceDown === 'function') {
                        movePieceDown();
                    }
                }, 150); // Good balance between responsiveness and control
            });
        ...
        // Set up arrow controls with appropriate keyCodes
        setupArrowControl(dpadLeft, 'ArrowLeft', 37);
        setupArrowControl(dpadRight, 'ArrowRight', 39);
        setupArrowControl(dpadUp, 'ArrowUp', 38);
        setupArrowControl(dpadDown, 'ArrowDown', 40);
        
        // Space button (Drop) - keep the existing implementation since it works
        if (btnSpace) {
            // Create new button to clear listeners
            const newBtnSpace = btnSpace.cloneNode(true);
            btnSpace.parentNode.replaceChild(newBtnSpace, btnSpace);
            
            // Touch start - begin drop
            newBtnSpace.addEventListener('touchstart', function(e) {
                e.preventDefault();
                newBtnSpace.classList.add('active');
                
                // Force speed change directly
            ...
            newBtnSpace.addEventListener('mouseup', function(e) {
                ...
                newBtnSpace.classList.remove('active');
                window.SOFT_DROP_ACTIVE = false;
                document.dispatchEvent(new KeyboardEvent('keyup', {key: ' ', keyCode: 32}));
            });
        }
    }, 800);
});

// Keep the original timer override for drop button
(function() {
    // Create global flag for soft drop
    window.SOFT_DROP_ACTIVE = false;
    
    // Store the original timing functions
    const originalSetTimeout = window.setTimeout;
    
    // Override setTimeout to check for soft drop
    ...
    window.dumpGameObjects = function() {
        console.log("Game objects:");
        for (const key in window) {
            if (typeof window[key] === 'function' && 
                (key.includes('move') || key.includes('piece') || key.includes('rotate') || 
                 key.includes('drop') || key.includes('game'))) {
                console.log("Function: " + key);
            }
        }
    };
    
    // Run the debugging helper after a short delay
    originalSetTimeout(window.dumpGameObjects, 2000);
})();
