...
function drawNextPiece(nextPiece) {
    // Skip drawing next piece since the nextPieceCanvas has been removed
    ...
    // Draw flame background for next piece preview
    drawFlameBackgroundForNextPiece(nextCtx);
    ...
    for (let row = 0; row < nextPiece.shape.length; row++) {
        for (let col = 0; col < nextPiece.shape[row].length; col++) {
            if (nextPiece.shape[row][col]) {
                ...
                // Add a 3D effect with lighter top/left and darker bottom/right
                nextCtx.fillStyle = 'rgba(255, 255, 255, 0.5)';
                nextCtx.fillRect(x, y, blockSize, 2); // Top highlight
                nextCtx.fillRect(x, y, 2, blockSize); // Left highlight
                
                nextCtx.fillStyle = 'rgba(0, 0, 0, 0.3)';
                nextCtx.fillRect(x, y + blockSize - 2, blockSize, 2); // Bottom shadow
                nextCtx.fillRect(x + blockSize - 2, y, 2, blockSize); // Right shadow
            }
        }
    }
    */
...
function createMobileControls() {
    ...
    // Add other control elements with appropriate classes
    // ...
}
